import { ChevronLeft, ChevronRight } from "lucide-react";
import Image from "next/image";

export function Main() {
  return (
    <main className="flex-1 p-6">
      <div className="flex items-center gap-3">
        <button className="rounded-full bg-black/40 p-1">
          <ChevronLeft />
        </button>
        <button className="rounded-full bg-black/40 p-1">
          <ChevronRight />
        </button>
      </div>
      <h1 className="font-semibold text-3xl mt-10">Good Afternoon</h1>
      <div className="grid grid-cols-3 gap-4 mt-4">
        <a
          href=""
          className="bg-white/5 rounded overflow-hidden flex items-center gap-4 group hover:bg-white/20 transition-colors"
        >
          <Image src="/1.webp" alt="capa de album" width={104} height={104} />
          <strong>Kill This Love</strong>
          <button className="w-12 h-12 rounded-full flex items-center pl-2 ml-auto mr-8 bg-green-400 invisible group-hover:visible ">
            <Image
              src="/play-fill.svg"
              alt="botao de play"
              width={32}
              height={32}
            />
          </button>
        </a>
        <a
          href=""
          className="bg-white/5 rounded overflow-hidden flex items-center gap-4 group hover:bg-white/20 transition-colors"
        >
          <Image src="/3.jpg" alt="capa de album" width={104} height={104} />
          <strong>Square Up</strong>
          <button className="w-12 h-12 rounded-full flex items-center pl-2 ml-auto mr-8 bg-green-400 invisible group-hover:visible ">
            <Image
              src="/play-fill.svg"
              alt="botao de play"
              width={32}
              height={32}
            />
          </button>
        </a>
        <a
          href=""
          className="bg-white/5 rounded overflow-hidden flex items-center gap-4 group hover:bg-white/20 transition-colors"
        >
          <Image src="/2.jpg" alt="capa de album" width={104} height={104} />
          <strong>Born Pink</strong>
          <button className="w-12 h-12 rounded-full flex items-center pl-2 ml-auto mr-8 bg-green-400 invisible group-hover:visible ">
            <Image
              src="/play-fill.svg"
              alt="botao de play"
              width={32}
              height={32}
            />
          </button>
        </a>
        <a
          href=""
          className="bg-white/5 rounded overflow-hidden flex items-center gap-4 group hover:bg-white/5 transition-colors"
        >
          <Image src="/4.jpg" alt="capa de album" width={104} height={104} />
          <strong>Square Two</strong>
          <button className="w-12 h-12 rounded-full flex items-center pl-2 ml-auto mr-8 bg-green-400 invisible group-hover:visible ">
            <Image
              src="/play-fill.svg"
              alt="botao de play"
              width={32}
              height={32}
            />
          </button>
        </a>
        <a
          href=""
          className="bg-white/5 rounded overflow-hidden flex items-center gap-4 group hover:bg-white/20 transition-colors"
        >
          <Image src="/5.jpg" alt="capa de album" width={104} height={104} />
          <strong>Square One</strong>
          <button className="w-12 h-12 rounded-full flex items-center pl-2 ml-auto mr-8 bg-green-400 invisible group-hover:visible ">
            <Image
              src="/play-fill.svg"
              alt="botao de play"
              width={32}
              height={32}
            />
          </button>
        </a>
        <a
          href=""
          className="bg-white/5 rounded overflow-hidden flex items-center gap-4 group hover:bg-white/20 transition-colors"
        >
          <Image src="/6.png" alt="capa de album" width={104} height={104} />
          <strong>The ALbum</strong>
          <button className="w-12 h-12 rounded-full flex items-center pl-2 ml-auto mr-8 bg-green-400 invisible group-hover:visible ">
            <Image
              src="/play-fill.svg"
              alt="botao de play"
              width={32}
              height={32}
            />
          </button>
        </a>
      </div>
      <h2 className="font-semibold text-2xl mt-10">Made for You</h2>

      <div className="grid grid-cols-5 gap-3 mt-4">
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play1.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play2.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play3.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play4.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/5.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play6.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play7.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play8.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play9.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
        <a className="bg-white/3 p-6 rounded-md flex flex-col gap-2 justify-center items-start hover:bg-white/10">
          <div className="flex flex-col gap-2 justify-center items-start">
            <Image
              src="/play10.jpg"
              alt="capa de album"
              width={240}
              height={120}
              className="rounded"
            />
            <strong className="font-semibold">Daily Mix</strong>
            <span className="text-sm text-zinc-500">
              Little Mix, Pussycat Dolls, Beyoncé, Rihanna ....
            </span>
          </div>
        </a>
      </div>
    </main>
  );
}
